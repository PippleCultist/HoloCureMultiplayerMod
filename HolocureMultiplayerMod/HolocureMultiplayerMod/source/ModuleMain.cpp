...
AurieStatus FindMemoryPatternAddress(const unsigned char* Pattern, const char* PatternMask, PVOID& outMemoryAddress)
{
	...
	if (!patternMatch)
	{
		LogPrint(LOG_SEVERITY_ERROR, "Couldn't find pattern %s", Pattern);
...
void YYGMLVariableGetValueHookFunc(RValue* arg1, void* arg2, void* arg3, RValue* arg4, void* arg5, void* arg6)
{
	...
	if (origYYGMLVariableGetValueFunc == nullptr)
	{
		...
		if (origYYGMLVariableGetValueFunc == nullptr)
		{
			...
			LogPrint(LOG_SEVERITY_ERROR, "Still couldn't get the original function for YYGMLVariableGetValueHookFunc. Expect undefined behavior.");
			return;
		}
	}
...
void YYGMLErrCheckVariableGetValueHookFunc(int arg1, void* arg2, void* arg3, void* arg4)
{
	...
	if (origYYGMLErrCheckVariableGetValueFunc == nullptr)
	{
		...
		{
			// Shouldn't ever occur, but might as well check for it
			LogPrint(LOG_SEVERITY_ERROR, "Still couldn't get the original function for YYGMLErrCheckVariableGetValueFunc. Expect undefined behavior.");
			return;
		}
	}
	if (hasConnected)
...
void YYGMLInstanceDestroyHookFunc(CInstance* Self, CInstance* Other, int arg3, void* arg4)
{
	...
	if (origYYGMLInstanceDestroyFunc == nullptr)
	{
		...
		{
			// Shouldn't ever occur, but might as well check for it
			LogPrint(LOG_SEVERITY_ERROR, "Still couldn't get the original function for YYGMLInstanceDestroyHookFunc. Expect undefined behavior.");
			return;
		}
	}
	if (hasConnected && isHost)
...
void initHooks()
{
	...
	if (isSteamInitialized)
	{
		if (!SteamAPI_Init())
		{
			...
			DbgPrintEx(LOG_SEVERITY_ERROR, "Couldn't initialize Steam api");
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Couldn't initialize functions from Steam API dll. Disabling Steam features.");
	}

	multiplayerMenuCreateFriendsSteamLobby->isVisible = isSteamInitialized;
	...
	for (int i = 0; i < numCommandLineArgs; i++)
	{
		...
		if (commandLine.compare("+connect_lobby") == 0)
		{
			if (i >= numCommandLineArgs - 1)
			{
				...
				DbgPrintEx(LOG_SEVERITY_ERROR, "Couldn't find steam lobby id");
			}
			else
			{
				...
				else
				{
					...
					DbgPrintEx(LOG_SEVERITY_ERROR, "Trying to join invalid steam lobby id");
				}
			}
		}
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_InputManager_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_Player_Mouse_53");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_Player_Draw_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_input_controller_object_Step_1");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_Enemy_Step_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_PlayerManager_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_PlayerManager_Step_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_PlayerManager_CleanUp_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_StageManager_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_StageManager_Step_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_CrateSpawner_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_CrateSpawner_Step_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_Cam_Step_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_cloudmaker_Alarm_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_Attack_Step_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_Attack_Collision_obj_BaseMob");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_Attack_Collision_obj_Obstacle");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_Attack_Collision_obj_Attack");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_Attack_CleanUp_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_Attack_Destroy_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_Player_Step_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_PlayerManager_Draw_64");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_EXP_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_EXP_Step_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_HoloCoinDrop_Step_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_PickUpable_Step_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_PickUpable_Collision_obj_Player");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_Hamburger_Collision_obj_Player");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_HoloCoinDrop_Collision_obj_Player");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_IdolPower_Collision_obj_Player");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_EXP_Collision_obj_Summon");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_HoloCoinDrop_Collision_obj_Summon");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_EXP_Collision_obj_EXP");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_Destructable_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_YagooPillar_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_Obstacle_Step_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_YagooPillar_Step_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_AttackController_Create_0");
		return;
	}
	if (!AurieSuccess(callbackManagerInterfacePtr->RegisterCodeEventCallback(MODNAME, "gml_Object_obj_PlayerManager_CleanUp_0", PlayerManagerCleanUpBefore, nullptr)))
	{
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_cautionattack_Step_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_caution_Step_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_PreCreate_Step_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_vfx_Step_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_afterImage_Step_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_afterImage_Alarm_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_holoBox_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_holoAnvil_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_goldenAnvil_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_GoldenHammer_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_Sticker_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_holoBox_Collision_obj_Player");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_holoAnvil_Collision_obj_Player");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_goldenAnvil_Collision_obj_Player");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_GoldenHammer_CleanUp_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_Sticker_Collision_obj_Player");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_Sticker_Step_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_TextController_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_TitleScreen_Draw_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_TitleScreen_Step_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_TitleCharacter_Draw_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_TitleScreen_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_TitleScreen_Mouse_53");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_Summon_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_Summon_Step_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_OreDeposit_Step_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_GetFish_Alarm_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_GetFish_Alarm_1");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_ShionPortal_Collision_obj_Player");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_AcerolaJuice_Collision_obj_Player");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_sapling_Collision_obj_Player");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_langOrb_Collision_obj_Player");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_hololiveMerch_Collision_obj_Player");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_PlayerManager_Other_23");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_Sticker_Alarm_1");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_CocoWeapon_Step_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_CocoWeapon_Collision_obj_Player");
		return;
	}
	if (!AurieSuccess(callbackManagerInterfacePtr->RegisterCodeEventCallback(MODNAME, "gml_Object_obj_MelMelCooking_Alarm_2", AllPickupableCollisionPlayerBefore, nullptr)))
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_MelMelCooking_Alarm_2");
		return;
	}
	if (!AurieSuccess(callbackManagerInterfacePtr->RegisterCodeEventCallback(MODNAME, "gml_Object_obj_MelMelCooking_Collision_obj_Player", AllPickupableCollisionPlayerBefore, nullptr)))
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_MelMelCooking_Collision_obj_Player");
		return;
	}
	if (!AurieSuccess(callbackManagerInterfacePtr->RegisterCodeEventCallback(MODNAME, "gml_Object_obj_Coronet_Collision_obj_Player", AllPickupableCollisionPlayerBefore, nullptr)))
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Object_obj_Coronet_Collision_obj_Player");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_input_binding_set");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_input_check");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_input_source_mode_set");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_input_source_clear");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_input_join_params_set");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_InitializeCharacter@gml_Object_obj_PlayerManager_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_SnapshotPrebuffStats@gml_Object_obj_Player_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_UpdatePlayer@gml_Object_obj_PlayerManager_Other_24");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_AddAttack@gml_Object_obj_PlayerManager_Other_24");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_input_direction");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_CanSubmitScore@gml_Object_obj_PlayerManager_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_input_source_using");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script___input_global");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_draw_text_outline");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_input_source_is_available");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_input_player_connected_count");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_input_source_get_array");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_input_source_detect_new");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_input_gamepad_is_connected");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_Stop@gml_Object_obj_Player_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_Init@gml_Object_obj_PlayerManager_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_Move@gml_Object_obj_Player_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_Pause@gml_Object_obj_PlayerManager_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script__CreateTakodachi@gml_Object_obj_AttackController_Other_14");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_glr_mesh_destroy");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_AddItem@gml_Object_obj_PlayerManager_Other_24");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_AddConsumable@gml_Object_obj_PlayerManager_Other_23");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_AddStat@gml_Object_obj_PlayerManager_Other_24");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_variable_struct_copy");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_ExecuteAttack@gml_Object_obj_AttackController_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_OnCollideWithTarget@gml_Object_obj_Attack_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_Die@gml_Object_obj_Obstacle_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_EliminateAttack@gml_Object_obj_PlayerManager_Other_24");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_RemoveItem@gml_Object_obj_PlayerManager_Other_24");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_RemovePerk@gml_Object_obj_PlayerManager_Other_24");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_ExecuteSpecialAttack@gml_Object_obj_InputManager_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_ApplyBuff@gml_Object_obj_AttackController_Other_11");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_Destroy@gml_Object_obj_holoAnvil_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_Destroy@gml_Object_obj_goldenAnvil_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_getAnvil@gml_Object_obj_PlayerManager_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_getGoldenAnvil@gml_Object_obj_PlayerManager_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_getSticker@gml_Object_obj_PlayerManager_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_Destroy@gml_Object_obj_Sticker_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_TakeDamage@gml_Object_obj_BaseMob_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_RollMod");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_AddEnchant@gml_Object_obj_PlayerManager_Other_24");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_getBox@gml_Object_obj_PlayerManager_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_AddCollab@gml_Object_obj_PlayerManager_Other_24");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_AddSuperCollab@gml_Object_obj_PlayerManager_Other_24");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_ApplyBuffs@gml_Object_obj_PlayerManager_Other_24");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_Confirmed@gml_Object_obj_TitleScreen_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_MouseOverButton");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_ReturnMenu@gml_Object_obj_TitleScreen_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_Return@gml_Object_obj_CharSelect_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_Select@gml_Object_obj_CharSelect_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_OnDeath@gml_Object_obj_BaseMob_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "gml_Script_CreateSummon@gml_Object_obj_MobManager_Create_0");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "struct_get_from_hash");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "struct_set_from_hash");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "instance_create_layer");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "sprite_delete");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "instance_exists");
		return;
	}
	...
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to register callback for %s", "ds_map_find_value");
		return;
	}
	...
	for (int i = 0; i < std::extent<decltype(VariableNamesStringsArr)>::value; i++)
	{
		...
		{
			LogPrint(LOG_SEVERITY_ERROR, "Failed to get hash for %s", VariableNamesStringsArr[i]);
		}
	...
	if (iResult != 0)
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to initialize winsock: %d\n", iResult);
		return;
	}
...
)
{
	...
	if (!AurieSuccess(status))
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to find memory address for %s", "YYGML_Variable_GetValue");
		return AURIE_MODULE_INITIALIZATION_FAILED;
	}
	...
	if (!AurieSuccess(status))
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to find memory address for %s", "YYGML_ErrCheck_Variable_GetValue");
		return AURIE_MODULE_INITIALIZATION_FAILED;
	}
	...
	if (!AurieSuccess(status))
	{
		LogPrint(LOG_SEVERITY_ERROR, "Failed to find memory address for %s", "YYGML_Instance_Destroy");
		return AURIE_MODULE_INITIALIZATION_FAILED;
	}
